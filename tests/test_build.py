...
def test_raw_command_builds_and_caches(tmp_path: Path) -> None:
...
def test_source_change_triggers_rebuild(tmp_path: Path) -> None:
    spec = command_spec(tmp_path)
    settings = make_settings()
...
def test_missing_output_is_an_error(tmp_path: Path) -> None:
    ...
    with _pytest.raises(BuildError, match="expected output"):
        run_build(spec, make_settings())


def test_unknown_backend_points_at_hook(tmp_path: Path) -> None:
    import pytest as _pytest

    from pytest_webots._core.errors import BuildError

    spec = ControllerSpec(robot="probe", path=tmp_path / "out.bin", build="cmake")
    with _pytest.raises(BuildError, match="pytest_webots_build_controller"):
        run_build(spec, make_settings())


def test_custom_make_path_is_used(tmp_path: Path) -> None:
...
def test_failing_command_raises_with_output(tmp_path: Path) -> None:
    import pytest as _pytest
    ...
    with _pytest.raises(BuildError, match="code 9") as excinfo:
        run_build(spec, make_settings())
